...
if [ ! -f "$ROOT/vscodium-insiders.nix" ]; then
  echo "ERROR: cannot find vscodium-insiders.nix in $ROOT"
  exit 1
fi
...
VSCODIUM_VER=$(curl -Ls -w %{url_effective} -o /dev/null \
               ...
               | awk -F'/' '{print $NF}')
sed -i "s/version = \".*\"/version = \"${VSCODIUM_VER}\"/" "$ROOT/vscodium-insiders.nix"
